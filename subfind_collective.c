	  ...
	  if(len > 0)
	      ...
	      else
		{
	      ...
	      subfind_col_determine_sub_halo_properties(ud, LocalLen, &SubMass,
							...
							&SubHalfMass, &SubMassTab[0], &SubPot);
	      tt1 = second();
	      ...
	      if(((GrNr - 1) % NTask) == ThisTask)
		{
          ...
          SubGroup[Nsubgroups].SubPot = SubPot;
...
void subfind_col_determine_sub_halo_properties(struct unbind_data *d, int num, double *totmass,
					       ...
					       MyIDType * mostboundid, double *halfmassrad, double *mass_tab, double *potsub)
{
  ...
  *totmass = mass;
  ...
  locdisp = loclx = locly = loclz = locpotsum = 0;
  ...
  for(i = 0, massloc = 0; i < num; i++)
    {
      ...
      locpotsum += P[part_index].Mass * P[part_index].p.Potential;
  ...
  MPI_Allreduce(&locpotsum, &potsum, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
